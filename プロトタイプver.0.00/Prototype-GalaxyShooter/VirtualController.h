...
namespace virtualControllerNS {
	...
	// スティックのデッドゾーンとして範囲の20%をデフォルトとする
	const float STICK_DEADZONE = 0.20f * 1000;
	// トリガーのデッドゾーンとして範囲の70%をデフォルトとする
	const float TRIGGER_DEADZONE = 0.70f * 1000;
...
private:
	...
	bool buttons[virtualControllerNS::BUTTONS::MAX_BUTTON];
	D3DXVECTOR2 leftStick;
	D3DXVECTOR2 rightStick;
	int controller_ID;
	int player_ID;
protected:
	...
	bool is_DeadZone(LONG value) {
		return value < virtualControllerNS::STICK_DEADZONE && value > -virtualControllerNS::STICK_DEADZONE ? true : false;
	}
	bool not_DeadZone(LONG value) {
		return !is_DeadZone(value);
	}
	//スティック情報のセット
...
public:
	...
	virtual void update()
	{
		...
		// 左スティック入力値を格納
		if (not_DeadZone(DCon->joyState.lX)) setX_Left_Stick(DCon->joyState.lX);
		if (not_DeadZone(DCon->joyState.lY)) setY_Left_Stick(DCon->joyState.lY);
		// 右スティック入力値を格納
		if (not_DeadZone(DCon->joyState.lRx)) setX_Right_Stick(DCon->joyState.lRx);
		if (not_DeadZone(DCon->joyState.lRy)) setY_Right_Stick(DCon->joyState.lRy);
		// 十字キーの設定
...
public:
	...
	virtual void update()
	{
		...
		if (not_DeadZone(DCon->joyState.lY)) setY_Left_Stick(DCon->joyState.lY);
		// 右スティック入力値を格納
		if (not_DeadZone(DCon->joyState.lZ))  setX_Right_Stick(DCon->joyState.lZ);
		if (not_DeadZone(DCon->joyState.lRz)) setY_Right_Stick(DCon->joyState.lRz);
		// 十字キーの設定
		setPOV(DCon->joyState.rgdwPOV[0]);
	};
};
